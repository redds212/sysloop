...
### 7.9 Admin
- Owner extension (2026-09-23): a separate **Do dyskusji** button (speech bubble) beside **Zgłoś błąd** creates a proposal/question about partnership agreements. The form lets the user choose **Cała pozycja** or **Wybrane odzywki** (at least one), plus a required description of up to 1,000 characters. Store the stable keys and a snapshot of selected call labels, never infer the selection from the message. No automatic grading or system edits. Admins get a separate **Do dyskusji** tab with **Do omówienia / W toku / Omówione**, reopening and deletion. Error reports remain in **Zgłoszenia**. Existing access rules remain: approved users create, admins read/manage. Migration 0008 adds `kind` (error/discussion, existing rows default to error) and `selected_lines` (JSON array of key/label snapshots; empty means the whole position) to `card_reports`. Missing migration must not silently save discussions as errors.
