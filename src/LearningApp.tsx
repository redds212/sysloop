...
export default function LearningApp({user,repository,journal,onLogout,onAdmin,account,preview=false}: {user:AppUser;repository:LearningRepository;journal:Journal;onLogout:()=>void;onAdmin?:()=>void;account?:ReactNode;preview?:boolean}) {
  ...
  const navigate=(next:View)=>{if(learning.busy||learning.error)return;setView(next);setDrawer(false);setFront(next.kind==='practice'||next.kind==='session')}
  ...
  return <div className="app-shell"><aside className="desktop-sidebar">{sidebar}</aside><div className="app-body">
    ...
    <main className="main-content">
      ...
      {(view.kind==='home'||view.kind==='settings')&&<>
        ...
        {view.kind==='settings'&&<><LearningSettings key={`${settings.dailyTarget}:${settings.mode}:${settings.timedMode}`} settings={settings} save={learning.updateSettings}/>{account}</>}
      </>}
      {view.kind==='session'&&card&&session&&<><div className="session-bar"><span>{session.inBuffer?'Poprawki':'Sesja dzienna'}</span><strong>{session.inBuffer?session.bufferIndex:session.index} / {session.inBuffer?session.buffer.length:session.session.slots.length}</strong><button className="text-button" disabled={busy} onClick={()=>navigate({kind:'home'})}>Wstrzymaj</button><progress value={session.inBuffer?session.bufferIndex:session.index} max={session.inBuffer?session.buffer.length:session.session.slots.length}/></div><CardView key={`${learning.day}:${session.index}:${session.bufferIndex}:${card.id}`} card={card} category={category} baseline={normalizeEntry(data.store[card.id])} timed={!!settings.timedMode} phase={session.inBuffer?'buffer':'main'} repository={repository} onRevealed={revealed} onRate={async a=>{await learning.answer(a);setFront(true)}}/></>}
