...
type View={kind:'hardPractice';ids:string[];visit:number;revisionTraining?:boolean}|{kind:'home'|'session'|'settings'|'hard'}|{kind:'practice'|'read'|'notes';id:string;visit:number}
export default function LearningApp({user,repository,journal,onLogout,onAdmin,account,preview=false}: {user:AppUser;repository:LearningRepository;journal:Journal;onLogout:()=>void;onAdmin?:()=>void;account?:ReactNode;preview?:boolean}) {
  ...
  return <div className="app-shell"><aside className="desktop-sidebar">{sidebar}</aside><div className="app-body">
    <header className="topbar"><button className="icon-button mobile-menu" onClick={()=>setDrawer(true)} aria-label="Otwórz menu">☰</button><span className="topbar-label">{view.kind==='session'?'Codzienna praktyka':view.kind==='practice'?'Ćwiczenie swobodne':view.kind==='read'?'Biblioteka':'System RJ-WG'}</span><span className="today-label">{new Date().toLocaleDateString('pl-PL',{day:'numeric',month:'long'})}</span></header>
    ...
    <main className="main-content">
      ...
      {view.kind==='hard'&&practiceAvailable&&<DifficultCards repository={repository} onStartChanges={ids=>navigate({kind:'hardPractice',ids,visit:Date.now(),revisionTraining:true})} cards={data.cards} categories={data.categories} attempts={data.attempts} stars={stars} busy={busy} onStar={learning.setStar} onRead={id=>open('read',id)} onStart={ids=>navigate({kind:'hardPractice',ids,visit:Date.now()})}/>}
      {view.kind==='hardPractice'&&practiceAvailable&&<HardPractice revisionTraining={view.revisionTraining} key={view.visit} cards={view.ids.flatMap(id=>data.cards.filter(c=>c.id===id))} categories={data.categories} attempts={data.attempts} stars={stars} onStar={learning.setStar} busy={busy} repository={repository} save={learning.saveHard} onFront={()=>setFront(true)} onRevealed={revealed} onFinish={()=>navigate({kind:'hard'})}/>}
      ...
      {view.kind==='session'&&card&&session&&<><div className="session-bar"><span>{session.inBuffer?'Poprawki':'Sesja dzienna'}</span><strong>{session.inBuffer?session.bufferIndex:session.index} / {session.inBuffer?session.buffer.length:session.session.slots.length}</strong><button className="text-button" disabled={busy} onClick={()=>navigate({kind:'home'})}>Wstrzymaj</button><progress value={session.inBuffer?session.bufferIndex:session.index} max={session.inBuffer?session.buffer.length:session.session.slots.length}/></div><CardView attempts={data.attempts} key={`${learning.day}:${session.index}:${session.bufferIndex}:${card.id}`} card={corrected?.card??card} scope={corrected?.scope} previousMissed={previousMissed} category={category} baseline={normalizeEntry(data.store[card.id])} timed={!!settings.timedMode} phase={session.inBuffer?'buffer':'main'} repository={repository} onRevealed={revealed} onRate={async a=>{await learning.answer(a);setFront(true)}}/></>}
  ...
  </div>{drawer&&<Modal title="System RJ-WG" onClose={()=>setDrawer(false)}>{sidebar}</Modal>}</div>
}
