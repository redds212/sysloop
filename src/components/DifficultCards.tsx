...
export function DifficultCards({cards,categories,attempts,stars,onStart,onRead,onStar,busy,repository,onStartChanges}: {
  ...
  repository:LearningRepository;onStartChanges:(ids:string[])=>void
}) {
  const [filter,setFilter]=useState<DifficultFilter|'recent'>('all')
  const items=difficultCards(cards,attempts,stars,filter==='recent'?'all':filter)
  return <section className="difficult-screen"><p className="eyebrow">Dodatkowa praktyka</p><h1>Trudne sekwencje</h1>
    ...
    <div className="difficult-filters" role="group" aria-label="Filtr trudnych sekwencji">{([['all','Wszystkie'],['starred','Moje ★'],['frequent','Częste błędy'],['recent','Ostatnio zmienione']] as const).map(([value,label])=><button key={value} className="secondary" aria-pressed={filter===value} onClick={()=>setFilter(value)}>{label}</button>)}</div>
    {filter==='recent'?<RecentChanges cards={cards} categories={categories} attempts={attempts} repository={repository} busy={busy} onStart={onStartChanges} onRead={onRead}/>:<>
