...
interface Props { card:Card;category?:Category;entry:SRSEntry;timed:boolean;repository:LearningRepository;save:(attempt:Attempt,entry:SRSEntry)=>Promise<void>;restore:(card:Card,entry:SRSEntry)=>Promise<void>;onRevealed:()=>void;onRepeat:()=>void;onRead:()=>void;onHome:()=>void }
export function FreePractice({card,category,entry,timed,repository,save,restore,onRevealed,onRepeat,onRead,onHome}:Props) {
