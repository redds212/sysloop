...
export function LearningSettings({settings,save,practiceAvailable=true}: {practiceAvailable?:boolean;settings:UserSettings;save:(next:UserSettings)=>Promise<void>}) {
  ...
  return <section className="panel settings"><h2>Ustawienia nauki</h2><form onSubmit={async e=>{e.preventDefault();setBusy(true);setMessage('');try{await save(draft);setMessage('Ustawienia zapisane.')}catch{setMessage('Nie udało się zapisać ustawień.')}finally{setBusy(false)}}}>
    ...
    <fieldset disabled={!practiceAvailable}><legend>Poprawki na końcu sesji</legend>{([['whole','Cała pozycja'],['missed','Tylko błędne odzywki']] as const).map(([value,label])=><label className="radio-row" key={value}><input type="radio" name="correctionMode" checked={(draft.correctionMode??'whole')===value} onChange={()=>setDraft({...draft,correctionMode:value})}/><span>{label}</span></label>)}<p className="muted">Jedna poprawka. Niezależnie od wyniku cała pozycja wraca jutro. Wybór dotyczy błędów ocenionych po zapisaniu ustawień.</p></fieldset>
