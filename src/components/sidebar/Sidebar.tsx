import type { AppUser, Card, Category, SRSEntry, SRSStore } from '../../types'
import { normalizeEntry } from '../../lib/srs'
import { AuctionView } from '../AuctionView'

function progressLabel(entry:SRSEntry) {
  if(entry.status==='NEW')return 'Nowa pozycja'
  if(entry.status==='LEARNING')return 'Do ponownej nauki'
  return `${entry.status==='MASTERED'?'Opanowane':'Utrwalanie'} · etap ${entry.consecutiveCorrect}/5${entry.nextReviewDate?` · powtórka ${entry.nextReviewDate}`:''}`
}
interface Props { user:AppUser; cards:Card[]; categories:Category[]; store:SRSStore; recommended:string[]; locked:boolean; onPractice:(id:string)=>void; onRead:(id:string)=>void; onNotes:(slug:string)=>void; onHard?:()=>void;onHome:()=>void; onSettings:()=>void; onLogout:()=>void; onAdmin?:()=>void }
export function Sidebar({user,cards,categories,store,recommended,locked,onPractice,onRead,onNotes,onHome,onSettings,onLogout,onAdmin,onHard}:Props) {
  ...
  return <div className="sidebar-inner"><button className="wordmark" onClick={onHome}>Sys<span>Loop</span></button><p className="sidebar-caption">TWÓJ SYSTEM, W PAMIĘCI</p>
    ...
    {onHard&&<button className="nav-button" onClick={onHard}>Trudne sekwencje <span>★</span></button>}
    ...
    <h2 className="eyebrow tree-title">Biblioteka systemu</h2><p className="tree-progress-legend"><span>Jasna zieleń · utrwalanie</span><span>Mocna zieleń · opanowane</span></p>
    ...
    {[...new Set(categories.map(c=>c.group))].map(group=><section key={group} className="tree-group"><h3>{group}</h3>{categories.filter(c=>c.group===group).map(category=>{
      ...
      return <details key={category.slug} className="category-branch"><summary><span>{category.name}</span><small>{seen} / {categoryCards.length}</small></summary>
        ...
        {[...new Set(categoryCards.map(c=>c.section))].map(section=><details key={section} className="section-branch"><summary className="section-title">{section||'Pozycje'}</summary>{categoryCards.filter(c=>c.section===section).map(card=><div key={card.id} className={`tree-position progress-${normalizeEntry(store[card.id]).status.toLowerCase()} progress-step-${normalizeEntry(store[card.id]).consecutiveCorrect}`} title={progressLabel(normalizeEntry(store[card.id]))}><button onClick={()=>onPractice(card.id)}><span className={`status-dot ${normalizeEntry(store[card.id]).status.toLowerCase()}`}/><AuctionView auction={card.auction} compact/>{card.context&&<small>{card.context}</small>}</button>{!locked&&<button className="read-icon" onClick={()=>onRead(card.id)} aria-label="Czytaj pozycję">↗</button>}</div>)}</details>)}
      </details>
    })}</section>)}
