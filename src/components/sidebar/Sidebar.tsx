...
interface Props { user:AppUser; cards:Card[]; categories:Category[]; store:SRSStore; recommended:string[]; locked:boolean; onPractice:(id:string)=>void; onRead:(id:string)=>void; onNotes:(slug:string)=>void; onHome:()=>void; onSettings:()=>void; onLogout:()=>void }
export function Sidebar({user,cards,categories,store,recommended,locked,onPractice,onRead,onNotes,onHome,onSettings,onLogout}:Props) {
